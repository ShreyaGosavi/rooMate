...
describe('ConversationService', () => {
  ...
  describe('getMyConversations', () => {
    it('should return conversations for a user', async () => {
      ...
      mockConversationModel.find.mockReturnValue({ sort: jest.fn().mockReturnValue({ exec: sortMock }) });
  ...
  describe('deleteConversation', () => {
    it('should delete conversation and messages if user is participant', async () => {
      ...
      expect(mockMessageModel.deleteMany).toHaveBeenCalledWith({ conversationId: 'conv-1' });
    });

    it('should throw NotFoundException if conversation not found', async () => {
      mockConversationModel.findById.mockResolvedValue(null);
      await expect(service.deleteConversation('bad-id', 'user-1')).rejects.toThrow(NotFoundException);
    });

    it('should throw ForbiddenException if user is not participant', async () => {
      mockConversationModel.findById.mockResolvedValue(mockConversation);
      await expect(service.deleteConversation('conv-1', 'user-3')).rejects.toThrow(ForbiddenException);
    });
  });
});
