...
export class ConversationService {
  ...
  async getOrCreate(userId: string, otherUserId: string): Promise<ConversationDocument> {
    const p1 = userId < otherUserId ? userId : otherUserId;
    const p2 = userId < otherUserId ? otherUserId : userId;
    const existing = await this.conversationModel.findOne({ participant1Id: p1, participant2Id: p2 });
    if (existing) return existing;
    return this.conversationModel.create({ participant1Id: p1, participant2Id: p2 });
  }
