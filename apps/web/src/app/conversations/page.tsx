...
export default function ConversationsPage() {
  ...
  const getOtherParticipant = (conv: any) => {
    ...
    return {
      userId: otherId,
      username: usernames[otherId] || otherId?.slice(0, 8),
    };
  };
  ...
  const filteredConvs = conversations.filter((c) => {
    ...
    return (
      other?.username?.toLowerCase().includes(search.toLowerCase()) || !search
    );
  });
    ...
    <div className="flex flex-col h-screen bg-white">
            ...
            filteredConvs.map((conv) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3.5 text-left transition-colors ${isActive ? "bg-[#f0f7f7] border-r-2 border-[#9fdbda]" : "hover:bg-[#f8fafa]"}`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-semibold text-[#061b32] truncate">
                        {other?.username || "Unknown"}
                      </p>
                      <p className="text-xs text-[#061b32]/40 shrink-0 ml-2">
                        {conv.updatedAt ? formatDate(conv.updatedAt) : ""}
                      </p>
                    </div>
                    <p className="text-xs text-[#061b32]/50 truncate mt-0.5">
                      {conv.lastMessage || "Start a conversation"}
                    </p>
                  </div>
                  {!openedConvs.has(conv._id) &&
      ...
      {activeConv ? (
          ...
          <div className="flex items-center justify-between px-4 md:px-6 py-4 border-b border-[#e2e8f0]">
            <div className="flex items-center gap-3">
              <button
                ...
                className="md:hidden text-[#061b32]/50 hover:text-[#061b32] mr-1"
              >
                <svg
                  width="20"
                  height="20"
              ...
              <div>
                <p className="text-sm font-semibold text-[#061b32]">
                  {getOtherParticipant(activeConv)?.username || "Unknown"}
                </p>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
            {msgLoading ? (
              <div className="flex items-center justify-center h-full">
                <div className="text-sm text-[#061b32]/40">
                  Loading messages...
                </div>
              </div>
            ) : (
              groupedMessages.map((group, gi) => (
                <div key={`${group.date}-${gi}`}>
                  <div className="flex items-center justify-center mb-4">
                    <span className="rounded-full bg-[#f0f7f7] px-3 py-1 text-xs text-[#061b32]/40">
                      {group.date}
                    </span>
                  </div>
                  <div className="space-y-2">
                    {group.msgs.map((msg: any) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`max-w-xs lg:max-w-md rounded-2xl px-4 py-2.5 ${isMine ? "bg-[#9fdbda]/20 rounded-tr-sm" : "bg-[#f8fafa] border border-[#e2e8f0] rounded-tl-sm"}`}
                          >
                            ...
                            >
                              ...
                              {isMine && (
                                <svg
                                  ...
                                  viewBox="0 0 24 24"
                                  fill="none"
                                  stroke={msg.read ? "#9fdbda" : "#061b32"}
                                  strokeWidth="2"
                                  opacity={msg.read ? 1 : 0.3}
                                >
          ...
          <div className="px-6 py-4 border-t border-[#e2e8f0]">
            <div className="flex items-center gap-3 rounded-2xl border border-[#e2e8f0] bg-[#f8fafa] px-4 py-3">
              ...
              <button
                onClick={sendMessage}
                disabled={!input.trim()}
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#9fdbda] text-[#061b32] hover:opacity-90 transition-opacity disabled:opacity-30"
              >
                <svg
                  width="16"
                  height="16"
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center bg-[#f8fafa]">
          ...
          <p className="text-sm text-[#061b32]/40 mt-1 max-w-xs">
