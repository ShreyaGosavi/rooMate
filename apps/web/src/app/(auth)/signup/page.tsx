...
export default function SignupPage() {
  ...
  const startPolling = (emailToCheck: string) => {
    ...
    pollRef.current = setInterval(async () => {
      try {
        const res = await api.get(`/api/auth/verification-status?email=${encodeURIComponent(emailToCheck)}`);
  ...
  const handleCompleteSignup = async () => {
    ...
    try {
      await api.post('/api/auth/signup', { email, username, password, phone: phoneDigits, gender });
    } catch (e: any) {
      setGeneralError(e?.response?.data?.message ?? 'Signup failed. Please try again.');
      setLoading(false);
      return;
    }
  ...
  const inputBase = 'w-full rounded-xl border bg-white pl-11 pr-4 py-3 text-sm text-[#061b32] placeholder:text-[#061b32]/30 focus:outline-none transition-colors';
    ...
    <div className="flex min-h-screen bg-white">
      ...
      <div className="hidden lg:flex lg:w-1/2 flex-col p-12 relative overflow-hidden min-h-screen bg-white">
        ...
        <div className="mt-10">
          ...
          <p className="mt-4 text-[#061b32]/50 text-sm leading-relaxed max-w-sm">
            Join thousands of students and professionals finding their perfect room and roommate on RooMate.
          </p>
        </div>
        <div className="flex-1 flex items-end pb-8">
          <Image src="/signup.svg" alt="Roommates" width={530} height={410} className="w-full" />
        </div>
        <div className="flex items-center gap-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[#9fdbda]/30">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#061b32" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
          </div>
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-6 py-12 bg-white">
        <div className="w-full max-w-md">
            ...
            {[
              ...
              <div key={s.n} className="flex items-center flex-1">
                <div className="flex flex-col items-center">
                  ...
                  <span className={`mt-1.5 text-xs font-medium ${step === s.n ? 'text-[#061b32]' : 'text-[#061b32]/30'}`}>{s.label}</span>
                </div>
                {i < 1 && (
                  <div className="flex-1 mx-3 mb-4">
                    <div className="h-[2px] w-full bg-[#e2e8f0] rounded-full overflow-hidden">
                      <div className={`h-full bg-[#9fdbda] transition-all duration-500 ${step === 2 ? 'w-full' : 'w-0'}`} />
                    </div>
                  </div>
                )}
              </div>
          ...
          {step === 1 ? (
            <>
              <h1 className="text-2xl font-bold text-[#061b32]">Create your account</h1>
              <p className="mt-1 text-sm text-[#061b32]/50">
                {sent ? 'Check your inbox and click the verification link.' : "Let's start with your email address."}
              </p>
              <div className="mt-8 space-y-4">
                ...
                {!sent ? (
                  ...
                  <>
                    <div className={`rounded-xl border p-4 text-sm ${verified ? 'border-[#9fdbda] bg-[#9fdbda]/10' : 'border-[#e2e8f0] bg-[#f8fafa]'}`}>
                      {verified ? (
                        ...
                        <span className="flex items-center gap-2 text-[#061b32]/60">
                          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#9fdbda" strokeWidth="2" className="animate-spin"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>
            ...
            <>
              <h1 className="text-2xl font-bold text-[#061b32]">Complete your profile</h1>
              <p className="mt-1 text-sm text-[#061b32]/50">Almost there — just a few more details.</p>
              <div className="mt-8 space-y-4">

                {/* Email - disabled */}
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[#061b32]">Email</label>
                  <div className="relative">
                    <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#061b32]/20">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
                    </span>
                    <input type="email" value={email} disabled className={`${inputBase} border-[#e2e8f0] bg-[#f8fafa] text-[#061b32]/40 cursor-not-allowed`} />
                  </div>
                </div>

                {/* Username */}
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[#061b32]">Username</label>
                  <div className="relative">
                    <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#061b32]/30">
                ...
                {/* Password */}
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[#061b32]">Password</label>
                  <div className="relative">
                    <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#061b32]/30">
                    ...
                    />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-[#061b32]/30 hover:text-[#061b32] transition-colors">
                      {showPassword ? (
                ...
                {/* Phone */}
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[#061b32]">Phone number</label>
                  <div className="relative">
                    <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#061b32]/30">
                ...
                {/* Gender */}
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[#061b32]">I am</label>
                  <div className="grid grid-cols-2 gap-3">
                    {([
                      { value: 'MALE', label: 'Male', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="7" r="4"/><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/></svg> },
                      { value: 'FEMALE', label: 'Female', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="4"/><path d="M12 12v9"/><path d="M9 18h6"/></svg> },
                    ] as const).map((g) => (
                      <button
                        key={g.value}
                        onClick={() => { setGender(g.value); setFieldErrors(p => ({ ...p, gender: '' })); }}
          ...
          <p className="mt-8 text-center text-sm text-[#061b32]/50">
            ...
            <Link href="/login" className="font-medium text-[#9fdbda] hover:opacity-80 transition-opacity">
              Log in
            </Link>
          </p>
        </div>
