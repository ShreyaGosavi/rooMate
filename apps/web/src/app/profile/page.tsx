...
export default function ProfilePage() {
  ...
  const fetchAll = async () => {
    try {
      const [userRes, listingsRes, savedRes, commRes] =
      ...
      if (listingsRes.status === "fulfilled")
        setMyListings(listingsRes.value.data || []);
    ...
    return (
      <div className="rounded-2xl border border-[#e2e8f0] bg-white overflow-hidden shadow-sm">
        <div className="relative h-44 bg-[#f0f7f7]">
          ...
          <div
            className={`absolute top-2 left-2 flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-semibold ${status.bg} ${status.text}`}
          >
            {status.label}
          </div>
        </div>
        <div className="p-3">
          ...
          <div className="mt-2 flex items-center justify-between">
            <span className="text-sm font-bold text-[#061b32]">
              ₹{p.rent?.toLocaleString()}
              <span className="text-xs font-normal text-[#061b32]/40">/mo</span>
            </span>
            ...
            <div className="mt-3 flex items-center justify-between border-t border-[#e2e8f0] pt-3">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`relative inline-flex h-4 w-8 items-center rounded-full transition-colors ${p.isAvailable ? "bg-[#9fdbda]" : "bg-[#e2e8f0]"}`}
                >
                  <span
                    className={`inline-block h-3 w-3 transform rounded-full bg-white shadow transition-transform ${p.isAvailable ? "translate-x-4" : "translate-x-1"}`}
                  />
                </button>
                <span
                  className={`text-xs ${p.isAvailable ? "text-[#9fdbda]" : "text-[#061b32]/30"}`}
                >
              ...
              <div className="flex gap-1">
                <Link
                  href={`/listings/${p.id}`}
                  className="flex h-7 w-7 items-center justify-center rounded-lg border border-[#e2e8f0] text-[#061b32]/40 hover:border-[#9fdbda] transition-all"
                >
                  <svg
                    width="12"
                    height="12"
                ...
                <button
                  ...
                  className="flex h-7 w-7 items-center justify-center rounded-lg border border-[#e2e8f0] text-[#061b32]/40 hover:border-red-300 hover:text-red-400 transition-all"
                >
                  <svg
                    width="12"
                    height="12"
          ...
          {!showToggle && (
            <div className="mt-3 flex items-center justify-between border-t border-[#e2e8f0] pt-3">
              <Link
                href={`/listings/${p.id}`}
                className="text-xs font-medium text-[#9fdbda] hover:opacity-80"
              >
                View →
              </Link>
              <button
                onClick={() => unsaveProperty(p.id)}
                className="text-xs text-[#061b32]/30 hover:text-red-400 transition-colors"
              >
                Remove
              </button>
            </div>
    ...
    <div className="min-h-screen bg-[#f8fafa]">
      ...
      <div className="flex flex-col md:flex-row min-h-[calc(100vh-64px)]">
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="flex flex-col items-center text-center mb-6">
                ...
                {user?.createdAt && (
                  <p className="text-xs text-[#061b32]/30 mt-1">
                    Member since{" "}
              ...
              <div className="grid grid-cols-3 gap-2 mb-6">
                {[
                  ...
                  <div
                    key={s.label}
                    className="rounded-xl bg-[#f8fafa] p-3 text-center"
                  >
                    <p className="text-lg font-bold text-[#061b32]">
                      {s.value}
                    </p>
              ...
              <div className="mt-auto space-y-2">
                <Link
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#9fdbda] py-2.5 text-sm font-semibold text-[#061b32] hover:opacity-90 transition-opacity"
                >
                  <svg
                    width="14"
                    height="14"
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-xl border border-[#e2e8f0] py-2.5 text-sm font-medium text-[#061b32]/60 hover:border-red-200 hover:text-red-400 transition-all"
                >
                  <svg
                    width="14"
                    height="14"
        ...
        <div className="flex-1 p-8 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-7 border-b border-[#e2e8f0] pb-0">
            {TABS.map((tab) => (
              <button
                ...
                className={`px-4 py-3 text-sm font-medium border-b-2 transition-all -mb-px ${activeTab === tab ? "border-[#9fdbda] text-[#061b32]" : "border-transparent text-[#061b32]/40 hover:text-[#061b32]"}`}
              >
                {tab}
                {tab === "My Listings" && myListings.length > 0 && (
                  <span className="ml-1.5 rounded-full bg-[#f0f7f7] px-1.5 py-0.5 text-xs">
                    {myListings.length}
                  </span>
                )}
                {tab === "Saved" && savedProperties.length > 0 && (
                  <span className="ml-1.5 rounded-full bg-[#f0f7f7] px-1.5 py-0.5 text-xs">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                {[
                  ...
                  },
                  {
                    ...
                    value: myListings.filter((p) => p.isAvailable).length,
                    icon: (
                      <svg
                        width="20"
                        height="20"
                  ...
                  >
                    ...
                    <p className="text-2xl font-bold text-[#061b32]">
                      {s.value}
                    </p>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {myListings.slice(0, 3).map((p) => (
                      <PropertyCard key={p.id} p={p} showToggle />
                    ))}
                  </div>
                </div>
              )}
              ...
              {myListings.length === 0 && (
                <div className="rounded-2xl border border-dashed border-[#e2e8f0] p-10 text-center">
                  <svg
                    className="mx-auto mb-3 text-[#9fdbda]"
                    width="36"
                    height="36"
                    viewBox="0 0 24 24"
                    fill="none"
                  ...
                  <p className="text-xs text-[#061b32]/40 mt-1">
                    List your first property to get started
                  </p>
                  <Link
                    href="/listings/create"
                    className="mt-4 inline-block rounded-xl bg-[#061b32] px-5 py-2 text-sm font-semibold text-white hover:opacity-90"
                  >
                    List a Property
                  </Link>
                </div>
          ...
          {activeTab === "My Listings" && (
            <div>
              <div className="flex items-center justify-between mb-5">
                ...
                <Link
                  href="/listings/create"
                  className="flex items-center gap-1.5 rounded-xl bg-[#9fdbda] px-4 py-2 text-xs font-semibold text-[#061b32] hover:opacity-90"
                >
                  <svg
                    width="12"
                    height="12"
              ...
              {myListings.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-[#e2e8f0] p-10 text-center">
                  ...
                  <Link
                    href="/listings/create"
                    className="mt-3 inline-block rounded-xl bg-[#061b32] px-5 py-2 text-sm font-semibold text-white hover:opacity-90"
                  >
                    List a Property
                  </Link>
                </div>
          ...
          {activeTab === "Saved" && (
            <div>
              <h3 className="font-semibold text-[#061b32] mb-5">
              ...
              {savedProperties.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-[#e2e8f0] p-10 text-center">
                  ...
                  <p className="text-sm text-[#061b32]/50">
              ...
              {communities.length === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {communities.map((c: any) => (
                    <Link
                      key={c._id || c.id}
                      href={`/communities/${c._id || c.id}`}
                      className="flex items-start gap-4 rounded-2xl border border-[#e2e8f0] bg-white p-4 hover:shadow-md hover:-translate-y-0.5 transition-all"
                    >
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[#061b32]/50 mt-0.5 line-clamp-2">
                          {c.description}
                        </p>
                        <p className="text-xs text-[#9fdbda] mt-1 font-medium">
