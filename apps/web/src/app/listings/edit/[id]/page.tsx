...
const AMENITY_OPTIONS = ['WIFI','LAUNDRY','WATER_SUPPLY','FURNISHED_BED','ATTACHED_BATHROOM','PARKING','GYM','POWER_BACKUP','CCTV','LIFT'];
...
export default function EditListingPage() {
  ...
  const fetchProperty = async () => {
    try {
      ...
      let rulesStr = '';
      try {
        const parsed = typeof p.rules === 'string' ? JSON.parse(p.rules) : p.rules;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const suitableArr = form.suitableFitFor.split(',').map(s => s.trim()).filter(Boolean);
    ...
    <div className="min-h-screen bg-[#f8fafa]">
      <nav className="sticky top-0 z-50 border-b border-[#e2e8f0] bg-white">
        <div className="mx-auto flex h-16 max-w-3xl items-center justify-between px-6">
          ...
          <Link href="/listings/my" className="text-sm text-[#061b32]/50 hover:text-[#061b32]">← My Listings</Link>
        </div>
      </nav>

      <div className="mx-auto max-w-3xl px-6 py-8">
        <h1 className="text-2xl font-bold text-[#061b32] mb-1">Edit Listing</h1>
        <p className="text-sm text-[#061b32]/50 mb-8">Update your property details.</p>

        <div className="space-y-6">
          {/* Title */}
          <div className="rounded-2xl border border-[#e2e8f0] bg-white p-6">
            <h2 className="text-sm font-semibold text-[#061b32] mb-4">Basic Info</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-[#061b32]/60 mb-1.5">Title</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={key}>
                    <label className="block text-xs font-medium text-[#061b32]/60 mb-1.5">{label}</label>
          ...
          {/* Availability */}
          <div className="rounded-2xl border border-[#e2e8f0] bg-white p-6">
            <h2 className="text-sm font-semibold text-[#061b32] mb-4">Availability</h2>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-medium text-[#061b32]/60 mb-1.5">Visiting Hours</label>
                <input value={form.visitingHrs} onChange={e => setForm(p => ({ ...p, visitingHrs: e.target.value }))}
                  placeholder="e.g. 10:00 - 18:00"
                  className="w-full rounded-xl border border-[#e2e8f0] px-4 py-2.5 text-sm text-[#061b32] focus:border-[#9fdbda] focus:outline-none" />
              </div>
              <div>
                <label className="block text-xs font-medium text-[#061b32]/60 mb-1.5">Available From</label>
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <span className="text-sm text-[#061b32]/70">{form.isAvailable ? 'Currently available' : 'Not available'}</span>
            </div>
          </div>

          {/* Amenities */}
          <div className="rounded-2xl border border-[#e2e8f0] bg-white p-6">
            <h2 className="text-sm font-semibold text-[#061b32] mb-4">Amenities</h2>
          ...
          {/* Suitable for */}
          <div className="rounded-2xl border border-[#e2e8f0] bg-white p-6">
            <h2 className="text-sm font-semibold text-[#061b32] mb-4">Suitable For</h2>
            <input value={form.suitableFitFor}
              onChange={e => setForm(p => ({ ...p, suitableFitFor: e.target.value }))}
          ...
          {/* Rules */}
          <div className="rounded-2xl border border-[#e2e8f0] bg-white p-6">
            <h2 className="text-sm font-semibold text-[#061b32] mb-4">House Rules</h2>
