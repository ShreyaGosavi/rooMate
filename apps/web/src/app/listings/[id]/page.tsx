...
const AMENITY_LABELS: Record<string, string> = {
  WIFI: 'Wi-Fi', AC: 'AC', PARKING: 'Parking', LAUNDRY: 'Laundry', GYM: 'Gym',
  POWER_BACKUP: 'Power Backup', WATER_SUPPLY: 'Water Supply', FURNISHED_BED: 'Furnished Bed',
      ...
      <nav className="sticky top-0 z-50 border-b border-[#e2e8f0] bg-white">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => router.back()} className="text-sm text-[#061b32]/50 hover:text-[#061b32] transition-colors">← Back</button>
            <Link href="/listings/create" className="rounded-xl bg-[#061b32] px-5 py-2 text-sm font-semibold text-white hover:opacity-90 transition-opacity">List Your Property</Link>
          </div>
        </div>
      </nav>
          ...
          <div className="flex-1 min-w-0">
                  ...
                  <div className="relative h-80 bg-[#f0f7f7]">
                    ...
                    <div className={`absolute top-4 left-4 flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold ${property.verificationStatus === 'VERIFIED' ? 'bg-[#9fdbda] text-[#061b32]' : 'bg-yellow-100 text-yellow-700'}`}>
                      {property.verificationStatus === 'VERIFIED' ? (
                        ...
                        <><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> Pending Verification</>
                      )}
                    </div>
                  </div>
                  {property.images.length > 1 && (
                    <div className="flex gap-2 p-3 bg-[#f8fafa] overflow-x-auto">
                      {property.images.map((img: string, i: number) => (
                        <button key={i} onClick={() => setActivePhoto(i)} className={`shrink-0 h-16 w-20 rounded-lg overflow-hidden border-2 transition-all ${activePhoto === i ? 'border-[#9fdbda]' : 'border-transparent'}`}>
            ...
            <div className="mt-6">
              ...
              <p className="mt-1 flex items-center gap-1.5 text-sm text-[#061b32]/50">
                ...
                {[property.addressLine1, property.locality, property.city, property.state].filter(Boolean).join(', ')}
              </p>
              {property.suitableFitFor?.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-2">
                  {property.suitableFitFor.map((p: string) => (
                    <span key={p} className="rounded-full bg-[#9fdbda]/20 border border-[#9fdbda]/30 px-3 py-1 text-xs font-medium text-[#061b32]">Near {p}</span>
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                ...
                { label: 'Sharing', value: property.sharing === 1 ? 'Private Room' : `${property.sharing} People` },
                { label: 'For', value: property.genderPreference === 'MALE' ? 'Boys Only' : 'Girls Only' },
              ].map(d => (
                <div key={d.label} className="rounded-xl border border-[#e2e8f0] bg-[#f8fafa] p-4 text-center">
                  <p className="text-xs text-[#061b32]/40 mb-1">{d.label}</p>
                  <p className="text-sm font-semibold text-[#061b32]">{d.value}</p>
                </div>
              ))}
            </div>
              ...
              <div className="mt-8">
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                  {property.amenities.map((a: string) => (
                    <div key={a} className="flex items-center gap-2 rounded-xl border border-[#e2e8f0] px-4 py-3 text-sm text-[#061b32]">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9fdbda" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
                      {AMENITY_LABELS[a] || a}
                    </div>
                  ))}
                </div>
                ...
                <div className="rounded-2xl border border-[#e2e8f0] bg-[#f8fafa] p-5 space-y-3">
                  {rules.map((r: string, i: number) => (
                    <div key={i} className="flex items-start gap-3">
                      <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#9fdbda]/20 text-xs font-bold text-[#061b32]">{i + 1}</span>
            ...
            <div className="mt-8">
              <div className="flex items-center justify-between mb-4">
                ...
                <a href={directionsUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 rounded-xl border border-[#e2e8f0] px-4 py-2 text-sm font-medium text-[#061b32] hover:border-[#9fdbda] transition-colors">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9fdbda" strokeWidth="2"><polygon points="3 11 22 2 13 21 11 13 3 11"/></svg>
                  Get Directions
                </a>
              </div>
            ...
            {similar.length > 0 && (
              <div className="mt-10">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {similar.map((p: any) => (
                    <Link key={p.id} href={`/listings/${p.id}`} className="group flex gap-3 rounded-2xl border border-[#e2e8f0] bg-white overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all">
                      <div className="w-24 shrink-0 bg-[#f0f7f7]">
                        {p.images?.[0] ? <img src={p.images[0]} alt={p.title} className="h-full w-full object-cover" /> : <div className="h-full flex items-center justify-center text-[#061b32]/20"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/></svg></div>}
                      </div>
                      <div className="flex-1 p-3">
                        <p className="text-sm font-semibold text-[#061b32] group-hover:text-[#9fdbda] transition-colors line-clamp-1">{p.title}</p>
                        <p className="text-xs text-[#061b32]/50 mt-0.5">{p.locality}, {p.city}</p>
                        <p className="text-sm font-bold text-[#061b32] mt-1">₹{p.rent.toLocaleString()}<span className="text-xs font-normal text-[#061b32]/40">/mo</span></p>
                      </div>
                    </Link>
                  ))}
                </div>
            ...
            <div className="sticky top-24 space-y-4">
              ...
              <div className="rounded-2xl border border-[#e2e8f0] bg-white p-5 shadow-sm">
                ...
                <div className="mt-3 space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-[#061b32]">₹{property.deposit?.toLocaleString()}</span>
                  </div>
                  {property.maintenance > 0 && (
                    <div className="flex justify-between">
                      <span className="text-[#061b32]/50">Maintenance</span>
                      <span className="font-medium text-[#061b32]">₹{property.maintenance?.toLocaleString()}/mo</span>
                    </div>
                  )}
                  {property.availableFrom && (
                    <div className="flex justify-between">
                      <span className="text-[#061b32]/50">Available From</span>
                      <span className="font-medium text-[#061b32]">{new Date(property.availableFrom).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                    </div>
                  )}
                  {property.visitingHrs && (
                    <div className="flex justify-between">
                      <span className="text-[#061b32]/50">Visit Timings</span>
                      <span className="font-medium text-[#061b32]">{property.visitingHrs}</span>
                    </div>
                  )}
                </div>
              ...
              <div className="rounded-2xl border border-[#e2e8f0] bg-[#f8fafa] p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-[#061b32]">Safe & Verified</p>
                    <p className="text-xs text-[#061b32]/50 mt-0.5">This listing has been reviewed by our team. Always visit in person before making any payment.</p>
                  </div>
                </div>
              </div>
            </div>
