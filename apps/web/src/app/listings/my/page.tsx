    ...
    <div className="min-h-screen bg-[#f8fafa]">
      ...
      <div className="px-8 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link
            href="/listings/create"
            className="flex items-center gap-2 rounded-xl bg-[#9fdbda] px-5 py-2.5 text-sm font-semibold text-[#061b32] hover:opacity-90 transition-opacity"
          >
            <svg
              width="14"
              height="14"
        ...
        {!loading && properties.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <Link
              href="/listings/create"
              className="mt-5 rounded-xl bg-[#061b32] px-6 py-3 text-sm font-semibold text-white hover:opacity-90 transition-opacity"
            >
              List a Property
            </Link>
          </div>
        ...
        {!loading && properties.length > 0 && (
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {properties.map((p) => {
              const status =
                statusConfig[p.verificationStatus] || statusConfig.PENDING;
              return (
                <div
                  key={p.id}
                ...
                >
                  <div className="relative h-48 bg-[#f0f7f7]">
                    ...
                    <div
                      className={`absolute top-3 left-3 flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold ${status.bg} ${status.text}`}
                    >
                  ...
                  <div className="p-4">
                    ...
                    <p className="mt-0.5 text-xs text-[#061b32]/50">
                    ...
                    <div className="mt-3 flex items-center gap-2 flex-wrap">
                      ...
                      <span className="rounded-full bg-[#f0f7f7] px-2.5 py-1 text-xs font-medium text-[#061b32]">
                    ...
                    <div className="mt-4 flex items-center justify-between border-t border-[#e2e8f0] pt-4">
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-[#061b32]/50">
                          Available
                        </span>
                        <button
                          onClick={() =>
                          ...
                          className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors duration-200 ${p.isAvailable ? "bg-[#9fdbda]" : "bg-[#e2e8f0]"} disabled:opacity-50`}
                        >
                          <span
                            className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform duration-200 ${p.isAvailable ? "translate-x-4" : "translate-x-1"}`}
                          />
                        </button>
                        <span
                          className={`text-xs font-medium ${p.isAvailable ? "text-[#9fdbda]" : "text-[#061b32]/30"}`}
                        >
                      ...
                      <div className="flex items-center gap-2">
                        <Link
                          href={`/listings/${p.id}`}
                          className="flex h-8 w-8 items-center justify-center rounded-lg border border-[#e2e8f0] text-[#061b32]/40 hover:border-[#9fdbda] hover:text-[#061b32] transition-all"
                        >
                          <svg
                            width="13"
                            height="13"
                        ...
                        <button
                          ...
                          className="flex h-8 w-8 items-center justify-center rounded-lg border border-[#e2e8f0] text-[#061b32]/40 hover:border-red-300 hover:text-red-400 transition-all disabled:opacity-50"
                        >
                          <svg
                            width="13"
                            height="13"
