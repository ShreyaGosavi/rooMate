  ...
  useEffect(() => {
    ...
    if (li) {
      api.get('/api/listings/my/saved').then(res => {
        ...
        setSavedIds(saved.map((p: any) => p.propertyId || p.Property?.id || p.id));
      }).catch(() => {});
    }
  }, []);
              ...
              {properties.map((p) => (
                <Link key={p.id} href={`/listings/${p.id}`} className="group rounded-2xl border border-[#e2e8f0] bg-white overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                  <div className="relative h-48 bg-[#f0f7f7]">
                    ...
                    {/* Save button */}
                    <button className={`absolute top-3 right-3 flex h-8 w-8 items-center justify-center rounded-full bg-white shadow-sm transition-colors ${savedIds.includes(p.id) ? 'text-red-400' : 'text-[#061b32]/40 hover:text-red-400'}`} onClick={async e => {
                      e.preventDefault();
